//this component is not used as the api is called from the backend
...
const Eleven2=()=>{
    ...
    const fetchApod=async()=>{
        try{
            const res=await axios.get(`https://api.nasa.gov/planetary/apod?api_key=${NasaKey}`)
            setApod(res)
        }
        catch(err){
            console.log(err)
    ...
    const fetchMars=async()=>{
        try{
            const res=await axios.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?sol=1000&api_key=${NasaKey}&page=1&per_page=14`)
    ...
    const fetchNeo=async()=>{
        try{
            const res=await axios.get(`https://api.nasa.gov/neo/rest/v1/neo/browse?api_key=${NasaKey}`)
    ...
    const fetchByDate=async()=>{
        try{
            const res=await axios.get(`https://api.nasa.gov/planetary/apod?api_key=${NasaKey}&date=${searchDate}`)
            setApodByDate(res)
       ...
       <ul className='nasaOptions'>
        ...
        }
        {
           ...
           <li> <p className='nasaP'>Do you want to see mars photos?</p>
                ...
                {marsPhotos.map((mars)=>(
                    <div>
                    <img key={mars.id} src={mars.img_src} alt={`Mars Photos-${mars.rover.name}`}/>
                    <p>{mars.earth_date}</p>
                    </div>
                ))}
                </div>
